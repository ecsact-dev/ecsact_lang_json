	...
	)
{
	...
	for(auto& field_id : ecsact::meta::get_field_ids(compo_id)) {
		...
		if(field_type.kind == ECSACT_TYPE_KIND_BUILTIN) {
			field_type_json["type"] = magic_enum::enum_name(field_type.type.builtin);
	...
	)
{
	...
	ctx.write(
		"\t\"source_file_path\": ",
		ecsact_meta_package_file_path(ctx.package_id),
		",\n"
	);

	ctx.write("\t\"name\": ", ecsact::meta::package_name(ctx.package_id), ",\n");
	ctx.write("\t\"imports\": [");
	ctx.write_each(
		", ",
		ecsact::meta::get_dependencies(ctx.package_id),
		[&](ecsact_package_id dep_id) {
			ctx.write("\"", ecsact::meta::package_name(dep_id), "\"");
		}
	);
	ctx.write("],\n");
	...
	{
		auto actions = "{}"_json;
		for(auto& action_id : ecsact::meta::get_action_ids(ctx.package_id)) {
			auto action = "{}"_json;
	...
	{
		auto systems = "{}"_json;
		for(auto& system_id : ecsact::meta::get_system_ids(ctx.package_id)) {
			auto system = "{}"_json;
...

	{
		...
		for(auto& component_id : ecsact::meta::get_component_ids(ctx.package_id)) {
...

	{
		...
		for(auto& transient_id : ecsact::meta::get_transient_ids(ctx.package_id)) {
